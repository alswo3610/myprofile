...
public class CleanMgr extends JFrame implements ActionListener {
   ...
   public JPanel createCad1Panel() {
      
      try {
         String sql = Query.SELECT_CLEANDATE_TABLE;
      ...
      try {
         String sql = Query.SELECT_CLEANWEEK_TABLE;

         PreparedStatement statement1 = connection.prepareStatement(sql);

         statement1.setString(1, "cad1급반");
         ...
         if (totalNames < 10) {
        	 JOptionPane.showMessageDialog(this, " 최소 10명 이상이어야 합니다.");
             
         }
         while (remainingPeople > 0) {
            // 남은 인원이 3명보다 작으면 그룹을 3명으로 구성
            int groupSize = Math.min(remainingPeople, 5);
   ...
   public JPanel createComPanel() {
      try {
         ...
         // 시작 날짜부터 종료 날짜까지의 날짜를 텍스트 형식으로 출력
         dateRangeText = "\n\n\n\n[   " + formattedStartDate + "(일)    ~    " + formattedEndDate + "(토)   ]";
      ...
      try {
         ...
         PreparedStatement statement1 = connection.prepareStatement(
               "SELECT user.user_id, user.user_name FROM user INNER JOIN class ON user.class_num = class.class_num WHERE class.class_name = ?");
         statement1.setString(1, "컴활반");
         ResultSet resultSet1 = statement1.executeQuery();
